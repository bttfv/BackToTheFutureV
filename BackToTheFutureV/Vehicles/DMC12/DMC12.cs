...
namespace BackToTheFutureV
{
    ...
    internal class DMC12
    {
        ...
        {
            if (Game.IsControlPressed(Control.VehicleDuck) && FusionUtils.PlayerPed.IsFullyInVehicle() && ((_isTimeMachine && TimeMachineHandler.CurrentTimeMachine.Mods.HoverUnderbody == InternalEnums.ModState.Off) || !_isTimeMachine) && GarageHandler.Status != InternalEnums.GarageStatus.Busy && !MenuHandler.IsAnyMenuOpen() && !isDucking)
            {
                FusionUtils.PlayerPed.Task?.PlayAnimation("veh@low@front_ds@idle_duck", "sit", -8f, 8f, -1, AnimationFlags.AllowRotation, 1f);
                isDucking = true;
                duckEnded = false;
            }
            else if (Game.IsControlPressed(Control.VehicleDuck) && FusionUtils.PlayerPed.IsFullyInVehicle() && ((_isTimeMachine && TimeMachineHandler.CurrentTimeMachine.Mods.HoverUnderbody == InternalEnums.ModState.Off) || !_isTimeMachine) && isDucking)
            {
