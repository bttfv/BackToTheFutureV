...
namespace BackToTheFutureV.Utility
{
    internal unsafe class VehicleBone
    {
        ...
        private VehicleBone(Vehicle vehicle, int index)
        {
            ...
            OriginalTranslation = Utils.GetBoneOriginalTranslation(vehicle, index);
            OriginalRotation = Utils.GetBoneOriginalRotation(vehicle, index);
        }
        ...
        public void RotateAxis(Vector3 axis, float degrees)
        {
            ...
            Matrix newMatrix = Matrix.Scaling(1.0f, 1.0f, 1.0f) * Matrix.RotationAxis(axis, Utils.DegreesToRadians(degrees)) * (*matrix);
            *matrix = newMatrix;
        }
        ...
        {
            int boneIndex = Utils.GetBoneIndex(vehicle, boneName);
            if (boneIndex == -1)
            {
                bone = null;
                return false;
